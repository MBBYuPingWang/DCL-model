function [E,grad]=DCCA_grad(VV,X1,X2,Y,F1,F2,K,rcov1,rcov2,dropprob1,dropprob2)
% [E,grad]=DCCA_grad(VV,X1,X2,F1,F2,K,rcov1,rcov2,dropprob1,dropprob2) 
%   computes the DCCA objective and gradient of the DCCA objective with 
%   respect to the network weights.
% 
% Inputs
%   VV: a long vector containing all weight parameters of view 1/view 2
%     networks.
%   X1/X2: inputs for view 1/view 2.
%   F1/F2: cell array containing architecture of view 1/view 2 network.
%   K: dimensionality of CCA projection.
%   rcov1/rcov2: regularization parameter for view 1/view 2.
%   droprob1/dropprob2: vector of drop-out probabilities at each layer of 
%     the view 1/view 2 network.
% 
% Outputs
%   E: objective function, i.e., -correlation + l2 penalty on weights.
%   grad: gradient of objective with respect to VV.
...
[XX1,R1]=forwardpass(X1,F1,dropprob1);          %% bookmark
...
if nargout==1
  corr=DCCA_corr(XX1{end},XX2{end},Y,K,rcov1,rcov2);
  E=-corr+R1+R2;
else
  [corr,G1,G2]=DCCA_corr(XX1{end},XX2{end},Y,K,rcov1,rcov2);
  E=-corr+R1+R2; G1=-G1; G2=-G2;                %% bookmark
...
% WX: R is the L2 penalty on weight matrix; XX is a array containing each layer rather than the output layer
% only.
...
for j=1:Nlayers
  ...
  if strcmp(F{j}.type,'conv')
    ...
    for filteridx=1:F{j}.numoutputmaps % numoutputmaps
...
% WX: dE = [de(:); dE], dE is the gradient descent step for VV;, delta_out is for the first layer.
